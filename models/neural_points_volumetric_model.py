...
class NeuralPointsVolumetricModel(BaseRenderingModel):
    ...
    def fill_invalid(self, output, input):
        ...
        if "coarse_depth" in output.keys():
            output["coarse_depth"] = output["coarse_depth"].view([1, -1, 1])
            coarse_depth_tensor = 0.01 * torch.ones([B, OR, output["coarse_depth"].shape[2]], dtype=output["coarse_depth"].dtype, device=output["coarse_depth"].device)
            coarse_depth_tensor[ray_inds[..., 0], ray_inds[..., 1], :] = output["coarse_depth"]
            output["coarse_depth"] = coarse_depth_tensor
