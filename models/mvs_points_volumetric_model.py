...
class MvsPointsVolumetricModel(NeuralPointsVolumetricModel):
    ...
    def set_points(self, points_xyz, points_embedding, points_color=None, points_dir=None, points_conf=None, Rw2c=None, eulers=None, editing=False, scores=None):
        if not editing:
            self.neural_points.set_points(points_xyz, points_embedding, points_color=points_color, points_dir=points_dir, points_conf=points_conf, parameter=self.opt.feedforward == 0, Rw2c=Rw2c, eulers=eulers, scores=scores)
        else:
    ...
    def query_embedding(self, HDWD, cam_xyz, photometric_confidence, imgs, c2ws, w2cs, intrinsics, cam_vid, pointdir_w=True, r2d2_feature=None, scores=None):
        img_feats = self.net_mvs.get_image_features(imgs)
        if r2d2_feature != None: # Preloading
            img_feats = [img_feats[0]]
            img_feats.append(r2d2_feature.unsqueeze(0))
            img_feats.append(scores.unsqueeze(0))
        else:
            img_feats = [img_feats[0]]
            localization_features = self.net_mvs.get_localization_features(imgs)
            img_feats.append(localization_features[0].unsqueeze(0))
            img_feats.append(localization_features[1].unsqueeze(0))
