...
import sys
parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../descriptor/r2d2'))
sys.path.append(parent_dir)
from extract_perpixel import extract_multiscale
from tools import common, dataloader
...
class NonMaxSuppression (torch.nn.Module):
    ...
    def forward(self, reliability, repeatability, **kw):
        ...
        maxima = 1
        # remove low peaks
        maxima *= (repeatability >= repeatability.min())
        maxima *= (reliability   >= reliability.min())
...
class MvsPointsModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.LocalizationNet = self.render_kwargs_train['network_localization']
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--data_vox_res', type=int, default=100)
    ...
    def extract_2d(self, img_feats, view_ids, layer_ids, intrinsics, c2ws, w2cs, cam_xyz, HD, WD, cam_vid=0):
        ...
        for vid in view_ids:
            ...
            for lid in layer_ids:
                ...
                elif img_feat.shape[1] == 1: # add score here
                    scores.append(warped_src_feat)
                else:
        ...
        scores = torch.cat(scores, dim=-1) if len(scores) > 0 else None
        return out_feats, colors, scores


    def get_localization_features(self, imgs):
        detector = NonMaxSuppression()
        imgs = imgs.squeeze().permute(1, 2, 0).cpu().numpy()
        img = dataloader.norm_RGB(imgs)[None].cuda()
        
        
        xys, desc, scores = extract_multiscale(self.LocalizationNet, img, detector,
        ...
        desc = desc.view([imgs.shape[0], imgs.shape[1], -1]).permute(2, 0, 1)
        scores = scores.view([imgs.shape[0], imgs.shape[1], -1]).permute(2, 0, 1)
        return desc, scores
    ...
    def query_embedding(self, HDWD, cam_xyz, photometric_confidence, img_feats, c2ws, w2cs, intrinsics, cam_vid, pointdir_w=False):
        ...
        for feat_str in getattr(self.args, feature_str_lst[cam_vid]):
            if feat_str.startswith("imgfeat"):
                ...
                twoD_feats, points_colors, points_scores = self.extract_2d(img_feats, view_ids, layer_ids, intrinsics, c2ws, w2cs, cam_xyz, HD, WD, cam_vid=cam_vid)
        ...
        return points_embedding, points_colors, points_dirs, points_conf, points_scores
