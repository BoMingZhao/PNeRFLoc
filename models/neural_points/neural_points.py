...
class NeuralPoints(nn.Module):
    ...
    def __init__(self, num_channels, size, opt, device, checkpoint=None, feature_init_method='rand', reg_weight=0., feedforward=0):
        ...
        if self.opt.load_points == 1:
            ...
            if checkpoint:
                ...
                self.scores = nn.Parameter(saved_features["neural_points.scores"]) if "neural_points.scores" in saved_features else torch.ones_like(self.points_conf, device=self.xyz.device)
            else:
            ...
            if self.points_embeding is not None:
                self.points_embeding.requires_grad = opt.feat_grad > 1
    ...
    def prune(self, thresh):
        ...
        if self.Rw2c is not None and self.Rw2c.dim() > 2:
            ...
            self.Rw2c.requires_grad = False
        if self.scores is not None:
            self.scores = nn.Parameter(self.scores[:, mask, :])
            self.scores.requires_grad = False
    ...
    def grow_points(self, add_xyz, add_embedding, add_color, add_dir, add_conf, add_eulers=None, add_Rw2c=None):
        ...
        if self.scores is not None:
            self.scores = nn.Parameter(torch.cat([self.scores, torch.zeros_like(add_conf[None, ...])], dim=1))
            self.scores.requires_grad = False
    ...
    def set_points(self, points_xyz, points_embeding, points_color=None, points_dir=None, points_conf=None, parameter=False, Rw2c=None, eulers=None, scores=None):
        ...
        if parameter:
            ...
            if self.opt.feat_grad > 0:
                points_embeding = nn.Parameter(points_embeding) # Save points embedding as a nn.Parameter
                points_embeding.requires_grad = self.opt.feat_grad > 1 # Do not optimize this Parameter
    ...
    def w2pers(self, point_xyz, camrotc2w, campos):
        ...
        xyz = torch.sum(camrotc2w[:, None, :, :] * point_xyz_shift[:, :, :, None], dim=-2)
        # print(xyz.shape, (point_xyz_shift[:, None, :] * camrot.T).shape)
        xper = xyz[:, :, 0] / (xyz[:, :, 2] + 0.00001)
        yper = xyz[:, :, 1] / (xyz[:, :, 2] + 0.00001)
        return torch.stack([xper, yper, xyz[:, :, 2]], dim=-1)
