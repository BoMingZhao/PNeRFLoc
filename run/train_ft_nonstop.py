...
def main():
    ...
    with torch.no_grad():
        ...
        if len([n for n in glob.glob(opt.checkpoints_dir + opt.name + "/*_net_ray_marching.pth") if os.path.isfile(n)]) > 0:
            ...
            if load_points == 2:
                points_xyz_all, points_xyz_origin_all = train_dataset.load_init_depth_points(device="cuda", vox_res=opt.data_vox_res)
            if load_points == 3:
            ...
            if opt.vox_res > 0:
                ...
                points_xyz_origin_all = [points_xyz_origin_all] if not isinstance(points_xyz_origin_all, list) else points_xyz_origin_all
                points_xyz_holder = torch.zeros([0,3], dtype=points_xyz_all[0].dtype, device="cuda")
                points_xyz_origin_holder = torch.zeros([0,1], dtype=points_xyz_all[0].dtype, device="cuda")
                for i in range(len(points_xyz_all)):
                    points_xyz = points_xyz_all[i]
                    points_xyz_origin = points_xyz_origin_all[i]
                    ...
                    points_xyz_origin = points_xyz_origin[sampled_pnt_idx, :]
                    print("after voxelize:", points_xyz.shape)
                    points_xyz_holder = torch.cat([points_xyz_holder, points_xyz], dim=0)
                    points_xyz_origin_holder = torch.cat([points_xyz_origin_holder, points_xyz_origin], dim=0)
                points_xyz_all = points_xyz_holder
                points_xyz_origin_all = points_xyz_origin_holder.to(torch.int64)
            ...
            # We directly record the source of each point 
            # and directly sample scene-agnostic localization features from the source.
            cam_ind = points_xyz_origin_all 
            ...
            points_embedding_all = torch.zeros([1, 0, featuredim - 7], device=unique_cam_ind.device, dtype=torch.float32)
            ...
            points_scores_all = torch.zeros([1, 0, 1], device=unique_cam_ind.device, dtype=torch.float32)
            ...
            for i in tqdm(range(len(unique_cam_ind))):
                ...
                c2w = batch["c2w"][0]
                w2c = torch.inverse(batch["c2w"][0]).cuda()
                ...
                if opt.pre_loading == 1:
                    H, W = HDWD[0], HDWD[1]
                    features = batch['r2d2_feature'].squeeze().view([H, W, -1]).permute(2, 0, 1).cuda()
                    scores = batch['r2d2_scores'].squeeze().view([H, W, 1]).permute(2, 0, 1).cuda()
                embedding, color, dir, conf, score = model.query_embedding(HDWD, cam_xyz_all[None,...], None, batch['images'].cuda(), c2w[None, None,...].cuda(), 
                                                                           w2c[None, None,...], intrinsic[:, None,...], 0, pointdir_w=True,
                                                                           r2d2_feature=features, scores=scores)
                ...
                points_scores_all = torch.cat([points_scores_all, score], dim=1)
